...
#        print("after action ", after, " duration ", after - before)
        ...
        print("SP3({}) rate {:5.2f}Hz | dur {:4.3f}s | a.motor {:5.2f} | a.steer {:5.2f} | m.vel {:4.3f}m/s | m.steer {:7.3f} | a {:2.0f} | b {:2.0f}".format(self.agent, 1000000000.0/since_last_laserscan.to_nsec(), duration.to_sec(), float(action['motor']), float(action['steering']), self._motor, self._steering, self._config_a, self._config_b))
...
if __name__ == '__main__':
    ...
    rospy.init_node('ab3_agent', anonymous=True)
