...
class RecurrentMPOLearner(acme.Learner):
  """MPO learner."""
  ...
  def __init__(
    ...
    self._variables = {
        ...
        'observation': self._target_observation_network.variables
    }
  ...
  def _step(self) -> types.Nest:
    ...
    observations, actions, rewards, discounts, dones, _ = inputs.data
    ...
    o_tm1 = observations[:-1]
    a_tm1 = tf.reshape(actions[:-1], shape=(-1, *actions.shape[2:]))
    o_t = observations[1:]
    a_t = actions[1:]
    r_t = tf.reshape(rewards[1:], shape=(-1,))
    d_t = tf.reshape(discounts[1:], shape=(-1,))

    batch_size, sequence_length = observations.shape[1], observations.shape[0]
    ...
    with tf.GradientTape(persistent=True) as tape:
      ...
      # network weights.
      initial_state = self._observation_network.initial_state(batch_size=batch_size)
      emb_tm1, s_Nm1 = self._observation_network.unroll(inputs=o_tm1, state=initial_state, sequence_length=sequence_length-1)
      emb_tm1 = tf.reshape(emb_tm1, shape=(-1, *emb_tm1.shape[2:]))
      # This stop_gradient prevents gradients to propagate into the target
      # observation network. In addition, since the online policy network is
      # evaluated at o_t, this also means the policy loss does not influence
      # the observation network training.
      initial_state = self._target_observation_network.initial_state(batch_size=batch_size)
      emb_t, s_N = self._target_observation_network.unroll(inputs=o_t, state=initial_state, sequence_length=sequence_length-1)
      emb_t = tf.stop_gradient(emb_t)
      emb_t = tf.reshape(emb_t, shape=(-1, *emb_t.shape[2:]))

      # Get action distributions from policy networks.
      online_action_distribution = self._policy_network(emb_t)
      target_action_distribution = self._target_policy_network(emb_t)

      # Get sampled actions to evaluate policy; of size [N, B, ...].
      sampled_actions = target_action_distribution.sample(self._num_samples)
      tiled_o_t = tf2_utils.tile_tensor(emb_t, self._num_samples)  # [N, B, ...]
      ...
      q_tm1 = self._critic_network(emb_tm1, a_tm1)  # [B, 1]
