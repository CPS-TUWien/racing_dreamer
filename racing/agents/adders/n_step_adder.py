...
from racing.agents.adders.file_adders import FileAdder

from dm_control import suite

from racing.environment.single_agent import Flatten


class NStepTransitionAdder(FileAdder):
...
class Selector(snt.Module):

    def __init__(self, key: str):
        self._key = key
        super().__init__(name='selector')

    def __call__(self, observation: NestedTensor) -> tf.Tensor:
        assert isinstance(observation, Mapping)
        assert self._key in observation
        return observation[self._key]


if __name__ == '__main__':
    adder = NStepTransitionAdder(directory='.', n_step=5, discount=0.99)
    env: Environment = suite.load('hopper', 'hop')

    action_network = snt.Sequential([
        Selector(key='position'),
        snt.nets.MLP(output_sizes=[64, env.action_spec().shape[0]])
    ])
    loop = EnvironmentLoop(env, actor=FeedForwardActor(action_network, adder=adder))
